...
function md5.sumhexa(luastr)
	local hash = ffi.new("uint8_t[16]");
	local p = ffi.cast("const char *", luastr);
	local ctx = MD5_CTX();

	MD5Init(ctx);
	MD5Update(ctx, p, #luastr);
	MD5Final(hash, ctx);

	return string.format("%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", hash[0], hash[1], hash[2], hash[3], hash[4], hash[5], hash[6], hash[7], hash[8], hash[9], hash[10], hash[11], hash[12], hash[13], hash[14], hash[15]);
end
